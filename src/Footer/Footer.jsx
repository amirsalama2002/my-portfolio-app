    ...
    <footer className="relative overflow-hidden bg-[#050814] text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
            ...
            <ul className="space-y-3">
              {nav.map((item) => (
                <li key={item.name}>
                  ...
                  >
                    {item.name}
                    <span className={`absolute ${isRTL ? 'right-0' : 'left-0'} -bottom-1 h-[2px] w-0 bg-blue-400 transition-all group-hover:w-full`} />
                  </Link>
                </li>
              ))}
            </ul>
        ...
        <div className="pt-10 text-center text-sm text-gray-400 space-y-2">
          <p>
            {isRTL ? "صُنع بـ" : "Made with"} <span className="text-red-500">❤</span> {isRTL ? "Top H Cleaning" : "Top H Cleaning"}
          </p>
          <p>
      ...
      <AnimatePresence>
        {showTop && (
          <motion.button
            ...
            exit={{ opacity: 0, y: 30, transition: { ease: "easeIn" } }}
